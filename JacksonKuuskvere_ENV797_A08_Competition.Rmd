...
```{r Auto ARIMA on deseasonal data SUBMITTED - submission 7}
...
```{r NNETAR1, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r NNETAR2, echo=TRUE, message=FALSE, warning=FALSE}

# Assuming temperature_clean is a dataframe with a column named daily_average containing humidity values
temperature_matrix <- as.matrix(temperature_clean$daily_average)  # Extract and convert to matrix
...
```{r NNETAR3, echo=TRUE, message=FALSE, warning=FALSE}
...
TBATS_fit <- tbats(load_ts, xreg=c(temperature_matrix, humidity_matrix))
